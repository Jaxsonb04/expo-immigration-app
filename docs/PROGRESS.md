...
| **6 — Build features (per-feature loop)** | 🟡 in progress | Local-data design loop implemented Home-first: design tokens, Home hub, metadata-only Profile/Vault, I-765 wizard shell, manual Tracker, Calendar, local reminder plan, and Forum safety shell. I-765 shared schema helpers + local non-PII autosave now cover reason, eligibility category, and review acknowledgement. Manual tracker receipt validation saves local government-side case summaries without USCIS sync claims. Calendar reminders support local acknowledge/snooze state while keeping Railway cron + Expo push as production-only contracts until auth/device tokens land. Forum now uses pseudonymous local categories/threads/posts, safe peer-support copy, reporting, and local author blocking without profile/PII mutation. Local-loop acceptance: tests/typecheck/lint/export pass plus `.maestro/filing-wizard.yaml`, `.maestro/tracker-manual-case.yaml`, `.maestro/calendar-reminder.yaml`, and `.maestro/forum-report.yaml` on the iPhone 17 simulator, no real PII persistence, no USCIS submission wording, news deferred. Backend/PII/PDF/push/moderation production wiring remains gated. |
...
*Last updated: 2026-06-27 — Stage 6 filing wizard autosave + tracker receipt persistence + calendar local reminders + forum safety shell verified with Maestro simulator QA.*
