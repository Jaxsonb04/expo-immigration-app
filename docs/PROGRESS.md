...
| **6 — Build features (per-feature loop)** | 🟡 in progress | Local-data design loop implemented Home-first: design tokens, Home hub, metadata-only Profile/Vault, I-765 wizard shell, manual Tracker, Calendar, and local reminder plan. I-765 shared schema helpers + local non-PII autosave now cover reason, eligibility category, and review acknowledgement. Manual tracker receipt validation saves local government-side case summaries without USCIS sync claims. Calendar reminders now support local acknowledge/snooze state while keeping Railway cron + Expo push as production-only contracts until auth/device tokens land. Local-loop acceptance: tests/typecheck/lint/export pass plus `.maestro/filing-wizard.yaml`, `.maestro/tracker-manual-case.yaml`, and `.maestro/calendar-reminder.yaml` on the iPhone 17 simulator, no real PII persistence, no USCIS submission wording, forum/news deferred. Backend/PII/PDF/push production wiring remains gated. |
...
*Last updated: 2026-06-27 — Stage 6 filing wizard autosave + tracker receipt persistence + calendar local reminders verified with Maestro simulator QA.*
