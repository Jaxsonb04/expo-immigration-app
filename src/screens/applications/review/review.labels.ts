...
export function formatValue(formType: FormType, key: string, raw: unknown): string {
	...
	if (key === 'dateOfBirth' || key === 'dateOfAdmission' || key === 'cardExpirationDate') {
...
export function blockerMessage(code: GroupBlockerCode): string {
	switch (code) {
		...
		case 'needs-preparer-parts':
			return "Because an interpreter or preparer was involved, the official form needs Parts 6–7, which this app doesn't prepare."
