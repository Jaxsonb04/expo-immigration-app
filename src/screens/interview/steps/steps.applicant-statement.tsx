  ...
  render: function Render({ form, formType }) {
    return (
      <View className="gap-card">
        ...
        <form.Subscribe
          selector={(state) => state.values.form.preparedSelfInEnglish}
        >
          {(answer) =>
            answer === "no" ? (
              <ScreeningStopNotice stop={interpreterStop(formType)} />
            ) : null
          }
        </form.Subscribe>
        {formType === "i90" && (
          <>
            ...
            <form.Subscribe
              selector={(state) => state.values.form.requestingAccommodation}
            >
              {(requesting) =>
                requesting === "yes" ? (
                  <View className="gap-card">
                      ...
                      validators={{
                        ...
                        onChange: ({ value, fieldApi }) => {
                          ...
                          const anyDetail =
                            value.trim() !== "" ||
                            fieldApi.form
                              .getFieldValue("form.accommodationBlindDetail")
                              .trim() !== "" ||
                            fieldApi.form
                              .getFieldValue("form.accommodationOtherDetail")
                              .trim() !== "";
                          return anyDetail
                            ? undefined
                            : "Describe at least one accommodation below";
                        },
                      }}
                    >
                      {(field) => (
