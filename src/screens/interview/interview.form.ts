...
const orEmpty = <T extends z.ZodType<string, string>>(schema: T) =>
...
export const fieldValidators = {
  ...
  aNumber: (kind: ApplicationKind): z.ZodType<string, string> =>
    kind === "initial"
      ? orEmpty(personFactsShape.shape.aNumber)
      : personFactsShape.shape.aNumber,
...
const citizenshipStep: StepDescriptor = {
  ...
  fieldPaths: [
    "personFacts.countryOfCitizenship",
...
const personalDetailsStep: StepDescriptor = {
  ...
  buildStepData: (values) => ({
    ...
    personFacts: dropEmpty({
      ...
      classOfAdmission: values.personFacts.classOfAdmission
        .trim()
        .toUpperCase(),
    ...
    return {
      form: {
        ...
        ...(requesting === "yes"
          ? dropEmpty({
              accommodationDeafSignLanguage:
                values.form.accommodationDeafSignLanguage,
...
const i765FinalStep: StepDescriptor = {
  ...
  buildStepData: (values, kind) => ({
    personFacts: dropEmpty({
      eligibilityCategory: values.personFacts.eligibilityCategory,
    }),
    form:
      kind === "replacement"
        ? (dropEmpty({
            replacementReason: values.form.replacementReason,
          }) as StepData["form"])
        : {},
  }),
};
 ...
 */
export function stepDescriptorsFor(
  formType: FormType,
...
export function initialStepIndex(
  ...
  const index = descriptorsByForm[formType].findIndex(
...
export const cardStatusOptions = [
  ...
  },
  {
    ...
    description:
      "Conditional permanent resident (for example CR1, CR2, CF1, CF2)",
  },
] as const;
...
export const heightFeetOptions = ["2", "3", "4", "5", "6", "7", "8"].map(
  (value) => ({
    value,
    label: `${value} ft`,
  }),
...
export const residencyPathOptions = [
  {
    ...
    description:
      "You entered the U.S. at a port of entry using an immigrant visa",
  },
  {
    value: "adjustmentOfStatus",
    label: "Adjustment of status",
    description:
      "You were already in the U.S. and USCIS granted your green card",
  },
] as const;
...
export const replacementReasonOptions = (formType: FormType) => [
  ...
  ...(formType === "i90"
    ? [{ value: "nameChange", label: "My name changed" }]
    : []),
];
