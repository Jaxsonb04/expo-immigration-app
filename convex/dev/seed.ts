...
function assertSeedEnabled() {
  if (env.DEV_SEED_ENABLED !== "true") {
    throw new Error(
      "Dev seed is disabled. Run: npx convex env set DEV_SEED_ENABLED true",
    );
  }
}
...
async function storeDemoBlobs(ctx: ActionCtx) {
  const store = (name: string) =>
    ctx.storage.store(
      new Blob([`demo placeholder: ${name}`], { type: "text/plain" }),
    );
  return {
...
export const seedDemo = action({
  ...
  handler: async (ctx) => {
    ...
    const summary: SeedSummary = await ctx.runMutation(
      internal.dev.seed.insertDemoData,
      {
        ownerId,
        storageIds,
      },
    ...
    await ctx.db.insert("applicationDrafts", {
      ...
      answers: {
        personFacts: {
          ...
          daytimePhone: "4155550112",
          gender: "female",
          motherGivenName: "Elena",
          fatherGivenName: "Jorge",
          classOfAdmission: "IR6",
          dateOfAdmission: "2015-06-10",
          heightFeet: "5",
          heightInches: "9",
          weightPounds: "170",
        ...
        form: { cardStatus: "permanentResident", replacementReason: "damaged" },
      },
      stepCompletion: completeAllSteps("i90"),
      updatedAt: now,
    });
